...
pub enum TradingDayTickerResponse {
    Full(Box<TradingDayTickerFull>),
    FullVec(Box<Vec<TradingDayTickerFull>>),
    Mini(Box<TradingDayTickerMini>),
    MiniVec(Box<Vec<TradingDayTickerMini>>),
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct TradingDayTickerFull {
    pub symbol: String,
